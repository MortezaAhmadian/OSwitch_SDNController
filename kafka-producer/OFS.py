...
class OFS:
    ...
    def make_connections_from_csv(self, csv_file_path):
        ...
        if pairs:
            ...
            print(f"PATCH request sent. Status Code: {status_code}, Response: {response_text}")
    ...
    def get_existing_cross_connects(self):
        """
        Retrieves the existing cross-connect pairs from the optical switch.
        ...
        try:
            ...
            for pair in root.findall('ns:pair', self.namespace):
                ...
                pairs.append({'port_in': int(ingress), 'port_out': int(egress)})

            return pairs

        except requests.exceptions.RequestException as e:
            print(f"Error retrieving cross-connects: {e}")
            return []
